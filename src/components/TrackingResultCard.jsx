...
export default function TrackingResultCard({ data }) {
  ...
  const getStatusDisplayText = (statusCode) => {
    switch (statusCode) {
      case 'delivered': return t('trackingCard.delivered');
      case 'in_transit': return t('trackingCard.inTransit');
      case 'out_for_delivery': return t('trackingCard.outForDelivery');
      case 'exception': return t('trackingCard.exception');
      default: return t('trackingCard.pending');
    }
  };
    ...
    <div className="bg-white rounded-3xl border border-slate-100 shadow-2xl overflow-hidden mb-8">
      <div className="p-8 bg-primary-blue text-white flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-200">{t('trackingCard.trackingNumber')}</p>
        ...
        <div className="flex flex-col md:items-end">
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-200 mb-1">{t('trackingCard.carrier')}</p>
      ...
      <div className="px-8 pt-6 pb-2">
        <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">{t('trackingCard.currentStatus')}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 p-8">
        <div>
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">{t('trackingCard.origin')}</p>
          <p className="text-lg font-black text-slate-900 leading-tight">{data.origin}</p>
        </div>
        <div>
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">{t('trackingCard.destination')}</p>
          <p className="text-lg font-black text-slate-900 leading-tight">{data.destination}</p>
        </div>
        <div>
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">{t('trackingCard.estimatedDelivery')}</p>
